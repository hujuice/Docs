...
class Docs
{
	...
	protected function _menuChildren($parents)
	{
		if (is_array($parents))
		{
			$params = array(); // Will be all '?'
			$ids = array(); // The values' list
			foreach ($parents as $parent)
			{
				if (isset($parent['id']))
				{
					...
					throw new \Exception('Malformed page (no id).');
			}

			$query = 'SELECT www_posts.ID as id, www_posts.post_parent as parent, www_postmeta.meta_value as label
						FROM www_posts
							LEFT JOIN www_postmeta ON www_postmeta.post_id = www_posts.ID
						WHERE www_posts.post_status = ?
							AND www_posts.post_type = ?
							AND www_postmeta.meta_key = ?
							AND www_posts.post_parent IN (' . implode(', ', $params) . ')
						ORDER BY www_posts.post_parent, www_posts.menu_order';
			$statement = $this->_db->prepare($query);

			$statement->bindValue(1, 'publish', \PDO::PARAM_STR);
			$statement->bindValue(2, 'page', \PDO::PARAM_STR);
			$statement->bindValue(3, 'titolobreve', \PDO::PARAM_STR);
			foreach($ids as $key => $id)
				$statement->bindValue($key + 4, $id, \PDO::PARAM_INT);
	...
	protected function _posts($ids, $full = false)
	{
		...
		$query = 'SELECT www_term_relationships.object_id as id, www_term_relationships.term_taxonomy_id as category_id, www_terms.name as label, www_term_taxonomy.taxonomy as type, www_categories.label as category
					...
					WHERE (www_term_taxonomy.taxonomy = :taxonomy OR www_categories.label IS NOT NULL)
						...
						AND www_term_relationships.object_id IN (:' . implode(',:', array_keys($bindings)) . ')
		...
		$query = 'SELECT post_id as id, meta_key, meta_value
					FROM www_postmeta
					WHERE post_id IN (:' . implode(',:', array_keys($bindings)) . ')';
		$statement = $this->_db->prepare($query);
		foreach ($bindings as $name => $param)
			$statement->bindValue($name, $param['value'], $param['type']);
		$info = $this->_execute($statement);
		...
		{
			$post['shortTitle'] = isset($meta[$post['id']]['titolobreve'])
									? $meta[$post['id']]['titolobreve']
									: '';

			$post['subTitle'] = isset($meta[$post['id']]['sottotitolo'])
									? $meta[$post['id']]['sottotitolo']
									: '';

			$post['period'] = isset($meta[$post['id']]['descrizioneperiodo'])
									? $meta[$post['id']]['descrizioneperiodo']
									: '';

			$post['description'] = isset($meta[$post['id']]['news'])
									? $meta[$post['id']]['news']
									: '';

			$post['abstract'] = isset($meta[$post['id']]['news_rss'])
            ...
            */
			...
			if ($full)
			{
				...
				$query = 'SELECT www_icl_translations.language_code as lang, www_icl_translations.element_id as id, www_postmeta.meta_value as label
							FROM www_icl_translations
								LEFT JOIN www_postmeta ON www_postmeta.post_id = www_icl_translations.element_id
							WHERE trid = (SELECT trid FROM www_icl_translations WHERE element_id = :post_id)
								AND www_icl_translations.language_code <> :lang
								AND www_postmeta.meta_key = :meta_key';
				$statement = $this->_db->prepare($query);
				$statement->bindValue('post_id', $post['id'], \PDO::PARAM_INT);
				$statement->bindValue('lang', $post['lang']);
				$statement->bindValue('meta_key', 'titolobreve');

				foreach ($result = $this->_execute($statement) as $translation)
				{
					$post['translations'][$translation['lang']] = array(
																		'id'	=> $translation['id'],
				...
				if (isset($meta[$post['id']]['docs_linkedSideposts']))
				{
					...
					$count = 1;
					foreach (unserialize($meta[$post['id']]['docs_linkedSideposts']) as $title => $box)
					{
						// Very strange!
					...
					if ($post['boxes']) // Not if empty...
					{
						$query = 'SELECT ID, post_content as body FROM www_posts WHERE post_status = :post_status AND ID IN (:' . implode(',:', array_keys($box_bindings)) . ') ORDER BY menu_order';
						$statement = $this->_db->prepare($query);
	...
	public function pages($lang)
	{
		...
		return array($lang => $this->_menuChildren($result));
	}
	...
	{
		$query = 'SELECT www_terms.name, www_term_taxonomy.count
					FROM www_terms
						LEFT JOIN www_term_taxonomy ON www_term_taxonomy.term_id = www_terms.term_id
						...
						)
	{
		...
		*/
		...
		if ($types)
		{
			...
			{
				$name = 'type' . $count;
				$bindings[$name] = array('value' => $type, 'type' => \PDO::PARAM_INT);
				$names[] = $name;
				$count++;
			}
		...
		if ($themes)
		{
			...
			{
				$name = 'theme' . $count;
				$bindings[$name] = array('value' => $theme, 'type' => \PDO::PARAM_INT);
				$names[] = $name;
				$count++;
			}
			$query = 'SELECT object_id
		...
		if ($regions)
		{
			...
			{
				$name = 'region' . $count;
				$bindings[$name] = array('value' => $region, 'type' => \PDO::PARAM_INT);
				$names[] = $name;
				$count++;
			}
			$query = 'SELECT object_id
		...
		if ($tags)
		{
			...
			$query = 'SELECT object_id
						FROM www_term_relationships
							LEFT JOIN www_terms ON www_terms.term_id = www_term_relationships.term_taxonomy_id
							LEFT JOIN www_term_taxonomy ON www_term_taxonomy.term_taxonomy_id = www_term_relationships.term_taxonomy_id
						WHERE www_term_taxonomy.taxonomy = :taxonomy AND www_terms.name IN (:' . implode(',:', $names) . ')';
		...
		if ($pubmin || $pubmax)
		{
			$query .= '
						AND www_postmeta.meta_key = ?';
			$bindings = array('value' => 'data_pubblicazione');
